...
def build_nsis(ctx):
    ...
    def req_wheel(x):
        ...
        for nw in no_wheels:
            if nw in x:
                return False
        return True
    # subprocess.check_call('pip-compile -o build.requirements.txt')    
    # requirements = list(filter(req_wheel, open('build-requirements.txt').read().strip().split('\n')))
    requirements = [e.split('#')[0].strip() for e in open('build.requirements.txt').read().strip().split('\n')]
    local_wheels = list(map(lambda y: os.path.join('dist', y),
    ...
    builder = nsist.InstallerBuilder(
        ...
        extra_files=[(get_configs_file_name(ctx), ''),]
    )
...
def build_package(ctx):
    ...
    shutil.copytree("miniperf/ui/dist", os.path.join(nsis_dir, "pkgs/miniperf/ui/dist"))
    shutil.copytree("miniperf/asserts", os.path.join(nsis_dir, "pkgs/miniperf/asserts"))
